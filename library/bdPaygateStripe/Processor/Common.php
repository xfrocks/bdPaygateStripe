...
abstract class bdPaygateStripe_Processor_Common extends bdPaygate_Processor_Abstract
{
        ...
        if (!empty($filtered['signature'])) {
            ...
            switch ($json['type']) {
                case 'charge.refunded':
                    return $this->_validateChargeRefunded($json, $transactionId, $paymentStatus, $transactionDetails,
                        $itemId, $amount, $currency);
                case 'invoice.payment_succeeded':
                    return $this->_validateInvoicePaymentSucceeded($json, $transactionId, $paymentStatus,
    ...
    protected function _validateChargeRefunded(
        array $json,
        &$transactionId,
        &$paymentStatus,
        &$transactionDetails,
        ...
        if (!empty($chargeMetadata['itemId'])) {
            $itemId = $chargeMetadata['itemId'];
            ...
            if ($customer instanceof \Stripe\Customer) {
                ...
                if (!empty($customerMetadata['itemId'])) {
                    $itemId = $customerMetadata['itemId'];
                }
            }
        }
    ...
    protected function _validateInvoicePaymentSucceeded(
        array $json,
        &$transactionId,
        &$paymentStatus,
        &$transactionDetails,
        ...
        $transactionDetails[bdPaygate_Processor_Abstract::TRANSACTION_DETAILS_SUBSCRIPTION_ID] = bdPaygateStripe_Helper_Sub::doPack($invoice->customer,
